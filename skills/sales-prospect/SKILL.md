...
| **官公庁・自治体・公共機関** | ドメインが .go.jp または .lg.jp、「令和〇年度」「予算要求」「入札公告」「随意契約」「LGWAN」等の表現、電子入札システムへのリンク | 公開入札プロセス（一般競争入札・指名競争入札・随意契約）、予算執行サイクル（4月始まり、補正予算）、長期検討・複数承認者・議会承認が必要なケース、個人情報保護法の公的機関版・マイナンバー法への対応 |

> **スコアリング注意：** 官公庁・自治体は入札プロセスが前提となるため、コンタクトアクセスや購買タイムラインの評価軸が民間企業とは異なる。標準スコアリングをそのまま適用せず、入札参加資格・調達スケジュール・予算執行時期を優先的に評価すること。
