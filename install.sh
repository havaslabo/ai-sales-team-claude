...
echo ""
echo -e "${BLUE}日本市場特化コマンド:${NC}"
echo ""
echo -e "  ${CYAN}/sales ringi <prospect>${NC}        稟議支援パッケージ生成（稟議書・FAQ・比較表）"
echo -e "  ${CYAN}/sales security-qa <company>${NC}   情報セキュリティ質問票対応支援"
echo -e "  ${CYAN}/sales event-followup <event>${NC}  展示会・セミナー後フォローアップ"
echo -e "  ${CYAN}/sales poc <company>${NC}           PoC（試験導入）提案書生成"
echo -e "  ${CYAN}/sales minutes <company>${NC}       商談議事録・確認メール生成"
echo -e "  ${CYAN}/sales channel <partner>${NC}       代理店・SIer・パートナー営業支援"
echo -e "  ${CYAN}/sales quote <company>${NC}         日本形式の見積書生成（インボイス対応）"
echo -e "  ${CYAN}/sales renewal <company>${NC}       契約更新・アップセル支援"
echo ""
