...
3. **求人情報**—特定ツールの経験要件から内部スタックが明らかになります。`"[企業名]" 採用 OR 求人 OR 中途採用` を検索し、役職説明のツール要件を調べてください。Wantedly（wantedly.com）や Green（green-japan.com）も確認してください。
...
1. `"[企業名]" 使用 OR 活用 OR "を導入" OR 連携 [ツールカテゴリ]` — 直接的な言及
2. `"[企業名]" site:stackshare.io OR site:builtwith.com` — テクノロジースタックデータベース
3. `"[企業名]" [競合製品名]` — 競合ツールの直接言及
4. `"[企業名]" 移行 OR 乗り換え OR 刷新 OR "から変更"` — 過去の乗り換え行動
5. `"[企業名]" 評判 OR 口コミ OR レビュー OR 比較 [ツールカテゴリ]` — ITreview・BOXIL での評価シグナル

#### 求人情報の詳細調査
現在の求人情報を検索し、ツール要件を抽出してください：
- `"[企業名]" 採用 OR 求人 [関連職種] site:wantedly.com OR site:green-japan.com OR site:doda.jp`
...
- **価格比較:** 自社ソリューションは incumbent より高いか安いか？価格で競争できるか、それとも価値で売る必要があるか？金額はすべて日本円（¥）で表記すること。
...
- **稟議プロセスの複雑さ:** 日本のBtoB営業では、複数の承認者・部門を経由する稟議が必要なことが多い。稟議の階層・承認ライン・決裁権者を把握すること。
...
1. **検出された競合他社の既知の限界:**
   - `[競合ツール] 問題点 OR 不満 OR デメリット OR 苦手` を検索する
   - 競合他社に関するよくある不満を ITreview（itreview.jp）、BOXIL（boxil.jp）、SaaS選び.com のレビューで確認する
...
- **サポートとサービスの問題:** 競合他社のカスタマーサポートが不十分という証拠（ITreview・BOXIL のレビュー、ソーシャルの苦情）
...
各 angle について：
- **Angle 名:** 覚えやすいラベル（例：「スケーラビリティギャップ」「インテグレーション優位」「価格パフォーマンス戦略」「稟議通過の容易さ」）
...
| 分類 | カテゴリ | 検出されたツール | 信頼度 | ソース | 定着度 |
|------|----------|--------------|-----------|--------|-------------|
| 国内競合 | [CRM] | [ツール名] | 高/中/低 | [求人情報、インテグレーションページ] | 深い/中程度/浅い |
| 海外競合（日本市場参入済み） | [アナリティクス] | [ツール名] | 高/中/低 | [ウェブサイトスクリプト] | 深い/中程度/浅い |
| SI・SES・内製化 | [基幹システム] | [SIer名 or 内製] | 高/中/低 | [ソース] | 深い/中程度/浅い |
...
**稟議突破の観点（他社より稟議が通りやすい理由）:**
1. [コスト合理性の根拠 — 数値で示す。例：競合比 ¥XX百万のコスト削減]
2. [リスク低減の根拠 — セキュリティ・コンプライアンス・導入実績]
3. [スモールスタート・段階導入の可否 — PoC・トライアルの提案方法]
4. [内製・SI代替の TCO 比較 — 総保有コストの優位性]
...
9. **競合は3種類で分類すること。** 国内競合・海外競合（日本市場参入済み）・ローカルベンダー（SI・SES）の3分類で整理し、大手SIer（アクセンチュア、NTTデータ、富士通、NEC等）の受託開発・内製化も競合として必ず検討すること。
10. **稟議突破の観点を必ず含めること。** 日本のBtoB営業では購買決定に稟議プロセスが関与する。Battlecardサマリーに「稟議突破の観点」セクションを必ず設け、他社より稟議が通りやすい理由を具体的な数値・実績・スモールスタート提案で示すこと。
11. **金額はすべて日本円（¥）で表記すること。** レビューサイトは ITreview（itreview.jp）、BOXIL（boxil.jp）、SaaS選び.com を優先し、採用情報は Wantedly、Green も確認すること。
